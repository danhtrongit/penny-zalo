...
function buildHelpText(): string {
  return [
    ...
    '/help - Xem hướng dẫn',
    '',
    '💬 Bạn vẫn có thể nhắn tự nhiên để ghi chi tiêu, xem lịch sử hoặc trò chuyện.',
  ].join('\n');
}
  ...
  private async handleEvent(event: ZaloEventPayload): Promise<void> {
    ...
    switch (event.event_name) {
      ...
      default:
        await this.reply(
          message.chat.id,
          '📎 Zalo Bot hiện hỗ trợ tốt nhất với tin nhắn text và ảnh. Nếu bạn cần nhập PDF, hãy dùng /login để mở Dashboard và tải file lên ở phần Cài đặt.',
        );
    }
  }
